    ...
    public class NativeMessagingHost
    {
        ...
        private async Task<object> ProcessCreateJobPacketAsync(MessageEnvelope envelope)
        {
            try
            {
                ...
                // Generate PDF
                var pdfPath = Path.Combine(jobFolder, payload.PdfFileName ?? "ad.pdf");
                await GeneratePdfAsync(payload, pdfPath);
        ...
        private async Task GeneratePdfAsync(CreateJobPacketPayload payload, string pdfPath)
        {
            ...
            // For now, save as HTML file (you can integrate a proper HTML-to-PDF library later)
            ...
            var textContent = GenerateTextContent(payload);
            await File.WriteAllTextAsync(txtPath, textContent, Encoding.UTF8);

            // TODO: Implement actual PDF generation using a library like iTextSharp, PuppeteerSharp, or similar
            // For now, create a placeholder PDF file
            await File.WriteAllTextAsync(pdfPath, "PDF generation not yet implemented. See accompanying .html and .txt files.", Encoding.UTF8);
        }
...
<body>
    ...
    <div class='footer'>
        ...
        private string GenerateTextContent(CreateJobPacketPayload payload)
        {
            var sb = new StringBuilder();
            sb.AppendLine("=".PadRight(80, '='));
            sb.AppendLine($"JOB APPLICATION RECORD");
            sb.AppendLine("=".PadRight(80, '='));
            sb.AppendLine();
            sb.AppendLine($"Title: {payload.Title}");
            sb.AppendLine($"Company: {payload.Company}");
            if (!string.IsNullOrEmpty(payload.Location))
                sb.AppendLine($"Location: {payload.Location}");
            if (!string.IsNullOrEmpty(payload.Pay))
                sb.AppendLine($"Pay: {payload.Pay}");
            sb.AppendLine();
            sb.AppendLine($"Captured: {payload.CapturedAtIso}");
            sb.AppendLine($"Source: {payload.SourceUrl}");
            if (!string.IsNullOrEmpty(payload.PostedAge))
                sb.AppendLine($"Posted: {payload.PostedAge}");
            if (!string.IsNullOrEmpty(payload.Applicants))
                sb.AppendLine($"Applicants: {payload.Applicants}");
            sb.AppendLine();
            sb.AppendLine("-".PadRight(80, '-'));
            ...
            var textDescription = StripHtml(payload.DescriptionHtml ?? "");
            sb.AppendLine(textDescription);
            
            sb.AppendLine();
            sb.AppendLine("-".PadRight(80, '-'));
            sb.AppendLine($"Generated by LinkedIn Job Logger on {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
